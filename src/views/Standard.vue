<template>
  <div id="standard-container">
    ...
    <v-container grid-list-md v-else id="standard-container--inner">
      <slot name="nav"/>
      <v-layout align-center justify-center row v-bind="filters">
        <v-btn
          class="mr-5"
          color="#e7e9ea"
          @click="sortHighest"
        ...
        <v-flex xs6 md3 v-for="(city, index) in cities" :key="index">
          <v-card class="ma-2" color="#e7e9ea">
            <v-card-text primary-title class="pa-2">
              <h3 class="text-capitalize text-sm-center text-xs-center">{{city.name}}</h3>
            </v-card-text>
            <v-card-text
            ...
            <v-card-text class="font-weight-bold">
              <p class="text-md-center text-xs-center">Teleport City Score: {{city.score.toFixed(2)}}</p>
            </v-card-text>
          </v-card>
        </v-flex>
      </v-layout>
